    ...
    <svg id={id} height={tartan?.imageSize} width={tartan?.imageSize} style={style} xmlns="http://www.w3.org/2000/svg">
      ...
      <defs>
        <mask id="threadHatchingMask" x="0" y="0" width="1" height="1">
          ...
          <ThreadHatching />
        <pattern id="sett" x={setX} y={setY} patternUnits="userSpaceOnUse" width={tartan !== null ? tartan?.getSetSize() * tartan?.scaleFactor : 0} height={tartan !== null ? tartan?.getSetSize() * tartan?.scaleFactor : 0} >
          <Weft />
          <Warp />
        </pattern>
      </defs>
      <rect x="0" y="0" height="100%" width="100%" fill="url(#sett)" filter={ isUseBlurFilter ? 'url(#blur)' : ''}></rect>
    </svg>
  )
}
